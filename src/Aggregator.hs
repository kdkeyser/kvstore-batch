...
add :: Aggregator -> KVOperation.KVOperation a -> IO (Future.FutureT IO a)
add aggregator@(entriesRef, _backend) action = do
    mVar <- newEmptyMVar
    let newAction = case action of
            KVOperation.Get key -> (GetAction key, \(GetResult result) -> putMVar mVar result)
            KVOperation.Put key value -> (PutAction key value, \(PutResult result) -> putMVar mVar result)
